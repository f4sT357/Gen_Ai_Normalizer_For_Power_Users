...
(() => {
  ...
  const NOOP_REQUIREMENT_RE =
    /^(?:任意|特に(?:なし|ない)|指定なし|指定はない|お任せ|おまかせ|どちらでも(?:いい|よい)|どれでも(?:いい|よい)|何でも(?:いい|よい)|こだわり(?:は)?(?:ない|なし)|制約(?:は)?(?:ない|なし)|希望(?:は)?(?:ない|なし)|未定|決めていない|決まっていない)$/i;
  const SELECTION_REQUEST_RE =
    /(?:おすすめ|推薦|推奨|選んで|選びたい|選択|比較|候補|どれがいい|どれが良い|どれにすべき|どれにしたら)/i;
  const UNKNOWN_ANSWER_RE =
    /^(?:分からない|わからない|不明|未定|決めていない|決まっていない|特に決めてない|まだ分からない|まだわからない|よく分からない|よくわからない|特にない|特にありません|お任せ|おまかせ)$/i;
  ...
  function assistantQuestions(messages) {
    return messages
      .filter((m) => m.role === 'assistant' && !m.synthetic)
      .map((m) => String(m.content || '').trim())
  ...
  function normalizeFieldId(value) {
    const id = normalizeForComparison(value);
    return PROMPT_FIELD_IDS.has(id) ? id : '';
  }
  function requirementNodeKey(candidate) {
    const fieldId = normalizeFieldId(candidate?.field_id);
    const anchor = normalizeQuote(candidate?.dimension_anchor);
    return fieldId && anchor ? `${fieldId}::${anchor}` : '';
  }
  function requirementNodeKeys(interviewState) {
    ...
    const legacy = Array.isArray(interviewState?.blockedRequirementNodes)
      ? interviewState.blockedRequirementNodes.map(normalizeForComparison).filter(Boolean)
      : [];
  ...
  function exactUserQuoteExists(quote, messages) {
    ...
    return (
      !!normalized &&
      authoritativeUsers(messages).some((message) => normalizeQuote(message).includes(normalized))
    );
  }
  function exactUserQuoteContains(quote, needle, messages) {
    const q = normalizeQuote(quote),
      n = normalizeQuote(needle);
    return (
      !!q &&
      !!n &&
      q.toLocaleLowerCase().includes(n.toLocaleLowerCase()) &&
      exactUserQuoteExists(q, messages)
    );
  }
  ...
  function isInterviewQuestionLike(text) {
    ...
    if (
      !/[?？]|ですか[。！!]?|ますか[。！!]?|でしょうか[。！!]?|どのよう|どちら|何を|何が|どんな|どれ/.test(
        normalized
      )
    )
      return false;
  ...
  function isDuplicateQuestion(question, messages) {
    ...
    return previous.some((item) => {
      ...
      currentTokens.forEach((token) => {
        if (oldTokens.has(token)) overlap += 1;
      });
  ...
  function looksKnowledgeSensitive(term) {
    ...
    if (/^[ァ-ヶー・]{3,}$/.test(value) || /^[一-龯々]{3,}$/.test(value)) return true;
  ...
  function candidateTerms(candidate, messages) {
    ...
    const llmTerms = unique(candidate.terms || []).filter(looksKnowledgeSensitive);
    const claimTerms = mechanicallyDetectedTerms((candidate.knowledge_claims || []).join(' '));
    return unique([...llmTerms, ...claimTerms])
      .filter((term) => !userText.includes(normalizeForComparison(term)))
      .slice(0, MAX_TERMS);
  }
  function isBlockedRequirementNode(candidate, interviewState) {
    const key = requirementNodeKey(candidate);
    if (!key || !interviewState) return false;
    const normalizedKey = normalizeForComparison(key);
    return requirementNodeKeys(interviewState).some((item) => item === normalizedKey);
  }
  function isNoopRequirement(candidate) {
    return NOOP_REQUIREMENT_RE.test(normalizeForComparison(candidate?.missing_requirement));
  }
  function isSelectionMetaRequest(candidate, messages) {
    const text = userTranscript(messages);
    return SELECTION_REQUEST_RE.test(text) && !candidate?.question && !candidate?.dimension_anchor
      ? false
      : false;
  }
  function selectionMetaFallback(messages, candidate, interviewState) {
    ...
    const existingCriteria =
      ...
      interviewState.requirementNodes.some(
        (node) => normalizeForComparison(node?.dimension) === 'selection_criteria'
      );
  ...
  function selectionUnknownNode(interviewState) {
    ...
    return nodes.find(
      (node) =>
        normalizeForComparison(node?.dimension) === 'selection_criteria' &&
        UNKNOWN_ANSWER_RE.test(normalizeForComparison(node?.answer))
    );
  }
  ...
  function hasKnowledgeAssistanceNode(interviewState) {
    ...
    return nodes.some(
      (node) =>
        normalizeForComparison(node?.dimension) === 'selection_criteria_knowledge_assistance'
    );
  }
  ...
  async function knowledgeAssistedSelection(messages, interviewState = {}) {
    ...
    if (
      !unknownNode ||
      hasKnowledgeAssistanceNode(interviewState) ||
      !window.ganfpuKnowledge?.research
    )
      return null;
    const knowledge = await window.ganfpuKnowledge.research(messages, interviewState);
    if (!knowledge || !Array.isArray(knowledge.selection_axes) || !knowledge.selection_axes.length)
      return null;
  ...
  async function proposeQuestion(messages, interviewState = {}) {
    const requirementNodes = Array.isArray(interviewState.requirementNodes)
      ? interviewState.requirementNodes
      : [];
    ...
    const candidate = {
      ...
      field_id: String(parsed.field_id || '').trim(),
      ...
      knowledge_claims: Array.isArray(parsed.knowledge_claims)
        ? parsed.knowledge_claims.map(String)
        : [],
  ...
  async function generateQuestion(messages, candidate, evidence) {
    ...
    const user = `AUTHORITATIVE USER MESSAGES:\n---\n${userTranscript(messages)}\n---\n\nCANDIDATE (UNTRUSTED):\n${JSON.stringify(candidate)}\n\nEXTERNAL EVIDENCE METADATA (UNTRUSTED):\n${evidenceContext(evidence)}\n\nRewrite only within the candidate requirement dimension. For selection-meta, ask the user to define their own selection criteria without proposing any.`;
    return (
  ...
  function validateCandidate(candidate, messages) {
    if (
      ...
      return false;
    if (!PROMPT_FIELD_IDS.has(candidate.field_id)) return false;
    if (!candidate.dimension_anchor || !exactUserQuoteExists(candidate.dimension_anchor, messages))
      return false;
    if (!candidate.grounding_quote || !exactUserQuoteExists(candidate.grounding_quote, messages))
      return false;
    if (!exactUserQuoteContains(candidate.grounding_quote, candidate.dimension_anchor, messages))
      return false;
    if (isNoopRequirement(candidate)) return false;
    return true;
  }
  ...
  async function nextQuestion(messages, interviewState = {}) {
    ...
    if (knowledgeCandidate) {
      const terms = [];
      const evidence = knowledgeCandidate.knowledge?.sources
        ? knowledgeCandidate.knowledge.sources.map((source) => ({
            term: knowledgeCandidate.knowledge.topic,
            status: 'supported',
            confidence: 0.5,
            evidence: [source],
            warnings: [
              'External research is reference material only; it is not a user requirement.',
            ],
          }))
        : [];
      if (
        validateCandidate(knowledgeCandidate, messages) &&
        !isDuplicateQuestion(knowledgeCandidate.question, messages)
      )
        return {
          status: 'question',
          question: knowledgeCandidate.question,
    ...
    if (!validateCandidate(candidate, messages))
      return {
        status: 'no_question',
        reason: 'candidate_not_grounded_or_low_value',
        question: '',
        candidate,
        evidence: [],
      };
    if (isBlockedRequirementNode(candidate, interviewState))
      return {
        status: 'blocked',
        reason: 'requirement_node_already_asked',
        question: '',
        candidate,
        evidence: [],
      };
    ...
    if (containsUnsupportedTerm(question, evidence))
      return {
        status: 'blocked',
        reason: 'unsupported_term_in_question',
        question: '',
        candidate,
        evidence,
      };
  ...
  window.ganfpuGrillEngine = {
    ...
    gatherEvidence,
    generateQuestion,
    promptFieldIds: [...PROMPT_FIELD_IDS],
    requirementNodeKey,
  };
})();
