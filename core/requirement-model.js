(() => {
  ...
  function validateRequirement(requirement, { allowCandidate = true } = {}) {
    ...
    if (!status || (!allowCandidate && status === 'candidate')) {
    ...
    if (status === 'confirmed' && !value) {
      return { valid: false, reason: 'missing_confirmed_value' };
    }

    if (status === 'unknown' && value) {
      return { valid: false, reason: 'unknown_has_value' };
    }

    if (status === 'not_required' && value) {
      return { valid: false, reason: 'not_required_has_value' };
    }

    const anchor = text(requirement.dimension_anchor || source.quote);
    if (!anchor) return { valid: false, reason: 'missing_dimension_anchor' };
    if (!source.quote.includes(anchor)) {
      return { valid: false, reason: 'anchor_not_in_source_quote' };
    }
  ...
  function normalizeRequirement(requirement, model) {
    const source = normalizeSource(requirement.source);
    const status = normalizeStatus(requirement.status) || 'candidate';
    return {
  ...
  function addRequirement(model, requirement, options = {}) {
    ...
    if (!validation.valid) {
      return { model: target, requirement: null, added: false, reason: validation.reason };
    }

    const existing = findRequirementByIdentity(target, normalized);
    if (existing) {
      return { model: target, requirement: existing, added: false, reason: 'duplicate_identity' };
    }
  ...
  function updateRequirement(model, id, patch = {}) {
    ...
    if (!validation.valid) {
      return { model: target, requirement: current, updated: false, reason: validation.reason };
    }

    const duplicate = findRequirementByIdentity({
      ...target,
      requirements: target.requirements.filter((item) => text(item?.id) !== text(id)),
    }, next);
    if (duplicate) {
      return { model: target, requirement: current, updated: false, reason: 'duplicate_identity' };
    }

    target.requirements = target.requirements.map((item) =>
      text(item?.id) === text(id) ? next : item
    );
  ...
  function cloneModel(model) {
    ...
    return {
      ...
      requirements: Array.isArray(target.requirements) ? target.requirements.map((item) => ({
        ...item,
