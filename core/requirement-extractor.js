(() => {
  ...
  async function extract(messages, model = {}) {
    ...
    let raw;
    try { raw = await adapter.request([{ role: 'system', content: 'You are a requirement extraction component. User-authored messages are authoritative evidence. Extract, do not invent.' }, { role: 'user', content: buildPrompt(messages, existing) }], 0.1); } catch (error) { throw error; }
  ...
  async function extractDelta({ userMessage, currentAction = null, model = {} } = {}) {
    ...
    const adapter = llm(); if (!adapter?.request) return [];
    let raw;
    try { raw = await adapter.request([{ role: 'system', content: 'You are a requirement extraction component. Extract only a user-grounded answer to the current action.' }, { role: 'user', content: buildDeltaPrompt(message, currentAction) }], 0.1); } catch (error) { throw error; }
    const parsed = parseJson(raw); if (parsed == null) return [];
