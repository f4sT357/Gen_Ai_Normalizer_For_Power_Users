  ...
  try {
    const res = await fetch(`${endpoint}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: grillMessages.filter(
          (m) => m.role !== 'system' || grillMessages.indexOf(m) === 0
        ),
        temperature: 0.7,
      }),
    });
    ...
    appendGrillMessage(
      'system',
      'Error: Failed to get response from local server. Make sure LM Studio is running and CORS is allowed.'
    );
  } finally {
...
Ensure all keys are present. For the format, tone, and length, you can map them to the best-matching existing option or provide custom descriptions. Output only the raw JSON.`;
  ...
  try {
    ...
    appendGrillMessage(
      'system',
      'Failed to map response to structured fields automatically. Copying AI text to Task field instead...'
    );
