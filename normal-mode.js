...
(() => {
    const COPY = {
        ...
        zh: {
            eyebrow: 'INTENT → PROMPT', title: '你希望 AI 做什么？',
            placeholder: '例如：找出并修复这段代码中的错误', start: 'Grill Me / Normalize',
            power: 'Power Mode', powerDesc: '手动编辑 Prompt Specification',
            needModel: '请先在 Power Mode 中选择 LM Studio 模型。', active: '关闭 Power Mode',
            model: '已选择 LM Studio 模型', noModel: '未选择 LM Studio 模型'
        },
        ko: {
            eyebrow: 'INTENT → PROMPT', title: 'AI에게 무엇을 시키고 싶나요?',
            placeholder: '예: 이 코드의 버그를 찾아 수정해줘', start: 'Grill Me / Normalize',
            power: 'Power Mode', powerDesc: 'Prompt Specification 직접 편집',
            needModel: '먼저 Power Mode에서 LM Studio 모델을 선택하세요.', active: 'Power Mode 닫기',
            model: 'LM Studio 모델이 선택됨', noModel: 'LM Studio 모델이 선택되지 않음'
        },
        es: {
            eyebrow: 'INTENT → PROMPT', title: '¿Qué quieres que haga la IA?',
            placeholder: 'Ej.: Encuentra y corrige los errores de este código', start: 'Grill Me / Normalize',
            power: 'Power Mode', powerDesc: 'Editar manualmente el Prompt Specification',
            needModel: 'Selecciona primero un modelo de LM Studio en Power Mode.', active: 'Cerrar Power Mode',
            model: 'Modelo de LM Studio seleccionado', noModel: 'Sin modelo de LM Studio seleccionado'
        },
        fr: {
            eyebrow: 'INTENT → PROMPT', title: 'Que voulez-vous faire faire à l’IA ?',
            placeholder: 'Ex. : Trouve et corrige les bugs de ce code', start: 'Grill Me / Normalize',
            power: 'Power Mode', powerDesc: 'Modifier manuellement le Prompt Specification',
            needModel: 'Sélectionnez d’abord un modèle LM Studio dans Power Mode.', active: 'Fermer Power Mode',
            model: 'Modèle LM Studio sélectionné', noModel: 'Aucun modèle LM Studio sélectionné'
        }
    };
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            @media (max-width: 700px) {
                #normal-mode { margin: 22px 0; }
                .normal-hero { padding: 26px 20px 22px; }
                .normal-actions { flex-direction: column; }
                #normal-model-status { min-height: 38px; justify-content: center; }
            }
        `;
    ...
    function startNormalGrill() {
        ...
        if (typeof selectedLMModel === 'undefined' || !selectedLMModel) {
            ...
            const c = copy();
            if (status) status.textContent = c.needModel;
            return;
        }
    ...
    function overrideApply() {
        // app.js currently omits reasoning/lang/hallucination from its Apply schema.
        // Replace only the final mapping function; all interview mechanics remain shared.
        window.applyGrillMeResult = async function() {
            ...
            try {
                const res = await fetch(`${endpoint}/chat/completions`, {
                    ...
                    body: JSON.stringify({ model: selectedLMModel, messages: grillMessages, temperature: 0.2 })
                });
                if (!res.ok) throw new Error('Failed to get structured JSON');
                const data = await res.json();
                const reply = data.choices[0].message.content.trim();
                if (chatLog && chatLog.lastChild && chatLog.lastChild.textContent.includes('Structuring requirements')) chatLog.removeChild(chatLog.lastChild);

                const match = reply.match(/\{[\s\S]*\}/);
                const parsed = JSON.parse(match ? match[0] : reply);
                const fieldIds = ['f-role','f-task','f-context','f-constraint','f-format','f-tone','f-length','f-reasoning','f-lang','f-hallucination'];
                fieldIds.forEach(fieldId => {
                    const val = typeof parsed[fieldId] === 'string' ? parsed[fieldId] : '';
    ...
    function init() {
        ...
        // Respect shared links containing structured form data: show Power Mode.
        if (location.hash.includes('data=')) togglePowerMode();

        // Mirror an existing autosaved task into Normal Mode after app.js initialization.
        setTimeout(() => {
            const input = document.getElementById('normal-intent');
            const task = document.getElementById('f-task');
            if (input && task && !input.value.trim() && task.value.trim()) input.value = task.value.trim();
            updateModelStatus();
        }, 350);
    }
