(() => {
  ...
  async function discover(messages, intent) {
    ...
    const searches = await Promise.all([`${subject} 選ぶ基準`, `${subject} 比較`, `${subject} selection criteria`].map(async (query) => { try { return await window.ganfpuEvidence.search(query, { limit: 6 }); } catch (error) { rethrowRateLimit(error); return { results: [] }; } }));
    ...
    } catch (error) { rethrowRateLimit(error); return null; }
  }
