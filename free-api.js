...
(() => {
    ...
    const PROVIDERS = {
        lmstudio: {
            label: 'LM Studio', endpoint: '', model: ''
        },
        groq: {
            label: 'Groq', endpoint: 'https://api.groq.com/openai/v1', model: 'openai/gpt-oss-20b'
        },
        openrouter: {
            label: 'OpenRouter Free', endpoint: 'https://openrouter.ai/api/v1', model: 'openrouter/free'
        }
    };
    ...
    async function fetchProviderModels() {
        ...
        try {
            ...
            console.error(e); showToast('Failed to fetch provider models.');
        }
    }
    ...
    function ensureReady() {
        if (provider === 'lmstudio') return !!(typeof selectedLMModel !== 'undefined' && selectedLMModel);
    ...
    function buildInitialConversation() {
        ...
        FIELDS.forEach(f => {
            ...
            if (val) filledFields[key] = val; else emptyFields.push(key);
        });
    ...
    async function startProviderGrill() {
        ...
        if (!input || !task || !input.value.trim()) { input?.focus(); return; }
        ...
        if (grillInput) { grillInput.value = ''; grillInput.disabled = false; }
            ...
            ids.forEach(id => {
                ...
                if (field.tagName === 'SELECT') {
                    ...
                    if (has) {
                        ...
                        field.value = 'custom';
                        const custom = el(id + '-custom'); if (custom) { custom.value = value; custom.style.display = 'block'; }
                    } else field.value = '';
    ...
    function replaceNormalHandlers() {
        ...
        if (send) {
            ...
            send.addEventListener('click', e => { e.stopImmediatePropagation(); });
        }
