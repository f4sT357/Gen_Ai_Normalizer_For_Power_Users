(() => {
  ...
  function authoritativeUserMessages(messages) { return (Array.isArray(messages) ? messages : []).filter((message) => message?.role === 'user' && !message?.synthetic).map((message) => ({ ...message, content: text(message.content) })).filter((message) => message.content); }
  function latestUserMessage(messages) { return authoritativeUserMessages(messages).slice(-1)[0] || null; }
  function completionAction(model) { return { type: 'complete', model, requirements: Array.isArray(model?.requirements) ? model.requirements : [], knowledge: Array.isArray(model?.knowledge) ? model.knowledge : [] }; }
  function knowledgeIsSufficient(model) { return Array.isArray(model?.knowledge) && model.knowledge.some((item) => Array.isArray(item?.findings) && item.findings.length > 0); }
  function hasInitialRequirementCue(message) { const value = text(message?.content); if (!value) return false; return /(?:予算|価格|金額|費用|期限|締切|納期|までに|以内|文字数|字以内|短く|長く|簡潔|詳細|箇条書き|表形式|JSON|Markdown|メール形式|敬語|カジュアル|丁寧|英語|日本語|中国語|韓国語|対象|読者|用途|目的|背景|前提|条件|制約|禁止|避け|含め|除外|出力|フォーマット|形式|トーン|語調|推論|理由|根拠|正確|事実|幻覚|ソース|引用|Python|JavaScript|TypeScript|コード|API|React|Next\.js|\bbudget\b|\bdeadline\b|\bformat\b|\btone\b|\baudience\b|\bcontext\b|\bconstraint\b|\blength\b|\blanguage\b|\breasoning\b|\bsource\b|\bcitation\b)/i.test(value); }
  function addRequirement(model, requirement) { if (typeof requirementApi()?.addRequirement !== 'function') return model; const result = requirementApi().addRequirement(model, requirement); return result?.model || model; }
  function seedInitialTask(model, messages) { const first = authoritativeUserMessages(messages)[0]; if (!first || text(model?.intent?.task_type) === 'unknown') return { model, seeded: false }; const api = requirementApi(); if (typeof api?.addRequirement !== 'function') return { model, seeded: false }; const value = text(first.content); const result = api.addRequirement(model, { field_id: 'f-task', dimension: 'task', dimension_anchor: value, value, status: 'confirmed', source: { type: 'user', message_id: first.id || null, quote: value } }); return { model: result?.model || model, seeded: result?.added === true }; }
  ...
  function isRateLimit(error) { return error?.code === 'RATE_LIMITED' || error?.status === 429; }
  ...
  function blockedResult(currentModel, currentDiscovery, error) { return { status: isRateLimit(error) ? 'rate_limited' : 'blocked', model: currentModel, discovery: currentDiscovery, action: null }; }
  async function step({ messages, model, discovery, currentAction }) {
    ...
    const intentApi = window.ganfpuIntentAnalyzer, extractor = window.ganfpuRequirementExtractor, discoveryApi = window.ganfpuRequirementDiscovery, knowledgeApi = window.ganfpuKnowledgeDiscovery;
    currentModel = { version: 1, intent: currentModel.intent || null, requirements: Array.isArray(currentModel.requirements) ? currentModel.requirements : [], knowledge: Array.isArray(currentModel.knowledge) ? currentModel.knowledge : [], pending: Array.isArray(currentModel.pending) ? currentModel.pending : [] };
    try { currentModel.intent = await resolveIntent(messages, currentModel, intentApi); } catch (error) { return blockedResult(currentModel, currentDiscovery, error); }
    if (!currentModel.intent) return { status: 'blocked', model: currentModel, discovery: currentDiscovery, action: null };
    const taskType = text(currentModel.intent?.task_type), authoritative = authoritativeUserMessages(messages);
