(() => {
  ...
  async function extractDelta({ userMessage, currentAction = null, model = {} } = {}) {
    ...
    const existing = Array.isArray(model?.requirements) ? model.requirements : [];
    let raw;
    try { raw = await adapter.request([{ role: 'system', content: 'You are a requirement extraction component. Extract only a user-grounded answer to the current action.' }, { role: 'user', content: buildDeltaPrompt(message, currentAction, existing) }], 0.1); } catch (error) { throw error; }
    const parsed = parseJson(raw); if (parsed == null) return [];
