(() => {
  ...
  function semanticRules() {
    return [
      ...
      'f-task / task: what the user wants the assistant to do or produce.',
      'f-context / usage: where, when, or in what situation the requested thing will be used.',
      'f-context / purpose: the purpose or intended use of the request or result.',
      'f-context / audience: who the result is for.',
      'f-constraint / budget: explicit price or budget limits.',
      'f-constraint / priority: explicit criteria the user wants to prioritize when choosing, comparing, or producing something.',
      'f-constraint / restriction: explicit restrictions, exclusions, prohibitions, or required conditions.',
      'f-format / format: explicit requested output format.',
      'f-tone / tone: explicit requested tone or style of expression.',
      'f-length / length: explicit length or size constraints.',
      'f-lang / language: explicit requested output language; conversation language alone is not evidence.',
      'Do not use f-task for a preference, budget, quality criterion, usage situation, or other constraint merely because it appeared in the same sentence as the task.',
      'When one user message contains multiple independently stated requirements, extract each requirement with the most specific applicable field/dimension.',
      'For a recommendation, a price ceiling belongs under f-constraint, and a desired product characteristic such as writing feel belongs under f-constraint / priority unless the user explicitly frames it as the task itself.',
    ].join('\n');
  }
