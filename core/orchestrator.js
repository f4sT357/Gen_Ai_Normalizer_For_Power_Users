(() => {
  ...
  function normalizeDiscovery(discovery = {}) { return { asked: Array.isArray(discovery.asked) ? discovery.asked.map((item) => ({ ...item })) : [], completed: Boolean(discovery.completed) }; }
  function normalizeModel(model, intent) {
    const api = window.ganfpuRequirementModel;
    const base = api?.createModel ? api.createModel(intent) : { version: 1, intent: intent || null, requirements: [], knowledge: [], pending: [] };
    return { ...base, ...clone(model), intent: intent || model?.intent || null,
      requirements: Array.isArray(model?.requirements) ? model.requirements.map((item) => ({ ...item, source: item?.source ? { ...item.source } : null })) : [],
  ...
  function recordAsked(discovery, action) {
    ...
    if (!current.asked.some((item) => text(item.id) === id)) current.asked.push({ id, question: text(action.question), target: action.target ? { ...action.target } : null });
    return current;
  }
  ...
  async function step({ messages = [], model = {}, discovery = {} } = {}) {
    const hasUserMessage = Array.isArray(messages) && messages.some((message) => message?.role === 'user' && !message?.synthetic && text(message.content));
    ...
    if (!currentModel.intent && intentApi?.analyze) currentModel.intent = await intentApi.analyze(messages);
    if (extractor?.extract) currentModel = mergeRequirements(currentModel, await extractor.extract(messages, currentModel));
    const taskType = text(currentModel?.intent?.task_type);
    if ((taskType === 'knowledge' || taskType === 'research' || taskType === 'recommendation') && knowledgeApi?.discover) {
      const discovered = await knowledgeApi.discover(messages, currentModel.intent);
      if (discovered) currentModel.knowledge = [discovered];
    }
    if ((taskType === 'knowledge' || taskType === 'research') && currentModel.knowledge.length) {
    ...
    if (discoveryApi?.nextAction) {
      ...
      if (action) return { action, model: currentModel, discovery: recordAsked(currentDiscovery, action) };
    }
    if (taskType === 'unknown') {
  ...
  window.ganfpuCore = Object.freeze({ step, normalizeDiscovery, normalizeModel });
})();
