(() => {
  ...
  function semanticRules() {
    return ['Semantic field mapping rules:','f-task / task: what the user wants the assistant to do or produce.','f-context / usage: where, when, or in what situation the requested thing will be used.','f-context / purpose: the purpose or intended use of the request or result.','f-context / audience: who the result is for.','f-constraint / budget: explicit price or budget limits.','f-constraint / priority: explicit criteria the user wants to prioritize when choosing, comparing, or producing something.','f-constraint / restriction: explicit restrictions, exclusions, prohibitions, or required conditions.','f-format / format: explicit requested output format.','f-tone / tone: explicit requested tone or style of expression.','f-length / length: explicit length or size constraints.','f-lang / language: explicit requested output language; conversation language alone is not evidence.','Do not use f-task for a preference, budget, quality criterion, usage situation, or other constraint merely because it appeared in the same sentence as the task.','When one user message contains multiple independently stated requirements, extract each requirement with the most specific applicable field/dimension.','For a recommendation, a price ceiling belongs under f-constraint, and a desired product characteristic such as writing feel belongs under f-constraint / priority unless the user explicitly frames it as the task itself.'].join('\n');
  }
  function buildPrompt(messages, existingRequirements) {
    return ['Extract only requirements explicitly stated by the user from the USER MESSAGES below.','Assistant messages are not authoritative evidence, but previous assistant questions may be used only to understand which requirement a user answer is responding to.','Assistant messages, system messages, model output, examples, and external knowledge must never become requirement values or sources.','Do not infer preferences, facts, technical specifications, recommendations, or unstated intent.','A requirement may be candidate only when it is a hypothesis; confirmed, unknown, and not_required MUST be directly supported by a user message.','When a user explicitly says they do not know, cannot decide, or does not understand an item asked by the previous assistant question, you may represent that target requirement as unknown. The source must still be the user\'s exact quote. Do not invent a value.','For confirmed/unknown/not_required requirements, source.type MUST be "user" and source.quote MUST be copied from the user message.', 'Use only these field IDs: f-role, f-task, f-context, f-constraint, f-format, f-tone, f-length, f-reasoning, f-lang, f-hallucination.', semanticRules(),'Return an array. Return [] when there are no new user-grounded requirements.','Do not repeat an existing requirement identity.',`Existing requirements:\n${JSON.stringify(existingRequirements || [])}`,`PREVIOUS ASSISTANT QUESTIONS (context only):\n${JSON.stringify(previousQuestions(messages))}`,`USER MESSAGES:\n${JSON.stringify(authoritativeUserMessages(messages))}`,'JSON schema for each item:','{"field_id":"f-constraint","dimension":"budget","dimension_anchor":"exact user phrase","value":"user-stated value","status":"confirmed","source":{"type":"user","message_id":"msg_01","quote":"exact user quote"}}','For unknown or not_required, value must be empty.','Output ONLY valid JSON.'].join('\n');
  }
  function buildDeltaPrompt(userMessage, currentAction, existingRequirements) {
    return ['Extract all requirements explicitly expressed by the latest USER MESSAGE.','The CURRENT ACTION identifies what requirement the assistant asked the user to clarify. Use it only to understand the conversational context; never treat the action itself as a user requirement.','The latest USER MESSAGE is the only authoritative evidence for new requirements.','A single answer may contain multiple independent requirements. Extract each explicitly stated requirement, including requirements that answer the current action and additional explicit constraints/preferences stated in the same message.','Do not infer preferences, facts, technical specifications, recommendations, or unstated intent.','If the user explicitly says they do not know, cannot decide, or does not understand the asked criterion, represent that target requirement as unknown with an empty value.','If the user explicitly says the asked criterion is unnecessary, represent that target requirement as not_required with an empty value.','For confirmed/unknown/not_required, source.type MUST be "user" and source.quote MUST be copied from the latest user message.','Use only these field IDs: f-role, f-task, f-context, f-constraint, f-format, f-tone, f-length, f-reasoning, f-lang, f-hallucination.',semanticRules(),'Return [] when the latest message does not establish any new requirement.','Do not repeat an existing requirement identity.','Return every distinct new requirement that is explicitly grounded in the latest user message; do not merge independent dimensions into one requirement.','CURRENT ACTION:',JSON.stringify(currentAction || null),`EXISTING REQUIREMENTS:\n${JSON.stringify(existingRequirements || [])}`,`LATEST USER MESSAGE:\n${JSON.stringify(userMessage || null)}`,'JSON schema for each item:','{"field_id":"f-constraint","dimension":"budget","dimension_anchor":"exact user phrase","value":"user-stated value","status":"confirmed","source":{"type":"user","message_id":"msg_02","quote":"exact user quote"}}','For unknown or not_required, value must be empty.','Output ONLY valid JSON.'].join('\n');
  }
  ...
  async function extract(messages, model = {}) {
    ...
    let raw;
    try { raw = await adapter.request([{ role: 'system', content: 'You are a requirement extraction component. User-authored messages are authoritative evidence. Extract, do not invent.' }, { role: 'user', content: buildPrompt(messages, existing) }], 0.1); } catch (error) { rethrowRateLimit(error); return []; }
  ...
  async function extractDelta({ userMessage, currentAction = null, model = {} } = {}) {
    ...
    const existing = Array.isArray(model?.requirements) ? model.requirements : [];
    let raw;
    try { raw = await adapter.request([{ role: 'system', content: 'You are a requirement extraction component. Extract only user-grounded requirements from the latest answer.' }, { role: 'user', content: buildDeltaPrompt(message, currentAction, existing) }], 0.1); } catch (error) { rethrowRateLimit(error); return []; }
    const parsed = parseJson(raw); if (parsed == null) return [];
    const candidates = Array.isArray(parsed) ? parsed : Array.isArray(parsed?.requirements) ? parsed.requirements : [];
    const accepted = [];
    for (const candidate of candidates) {
      ...
      if (accepted.some((item) => api.requirementIdentity(item) === api.requirementIdentity(normalized))) continue;
