...
(() => {
  const UNKNOWN_ANSWER_RE = /(?:分からない|わからない|分からん|わからん|よく分からん|よくわからん|不明|未定|決めていない|決めてない|決まっていない|決まってない|詳しくない|よく知らない|知らない|任せる|お任せ|おまかせ)/i;
  ...
  window.ganfpuGrillEngine.nextQuestion = async function(messages, interviewState = {}) {
    ...
    const selectionNode = nodes.find((node) =>
      normalized(node?.dimension) === 'selection_criteria' && UNKNOWN_ANSWER_RE.test(normalized(node?.answer))
    );
