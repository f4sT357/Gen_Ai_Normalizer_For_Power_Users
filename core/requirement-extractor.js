(() => {
  ...
  async function extractDelta({ userMessage, currentAction = null, model = {} } = {}) {
    ...
    const existing = Array.isArray(model?.requirements) ? model.requirements : [];
    let raw;
    try { raw = await adapter.request([{ role: 'system', content: 'You are a requirement extraction component. Extract only a user-grounded answer to the current action.' }, { role: 'user', content: buildDeltaPrompt(message, currentAction, existing) }], 0.1); } catch (error) { rethrowRateLimit(error); return []; }
    const parsed = parseJson(raw); if (parsed == null) return [];
    const candidates = Array.isArray(parsed) ? parsed : Array.isArray(parsed?.requirements) ? parsed.requirements : [];
    const accepted = [];
    for (const candidate of candidates.slice(0, 1)) { const normalized = { ...candidate, source: candidate.status === 'candidate' ? (candidate.source && typeof candidate.source === 'object' ? { ...candidate.source } : null) : { type: 'user', message_id: message.id, quote: text(candidate.source?.quote) || message.content } }; const valid = validateCandidate(normalized, [message]); if (!valid.valid) continue; if (api.findRequirementByIdentity(model, normalized)) continue; accepted.push(normalized); }
    return accepted;
  }
